...
| Localization verifier | Fixed | XML parsing, comments, escaped content, duplicate detection, missing keys, and extra-key reporting across five packs | Added `scripts/verify_strings.py` | All packs currently contain 589 matching keys. |
...
| Button action feedback | Fixed | Settings changes, Settings save, Premium unlock, workout start/pause/resume/finish, water add, measurement save, reminder save, feedback submit, share launch, destructive confirmations, and Coming Soon placeholders | Added localized feedback strings, local snackbars, persistent success text, and `docs/BUTTON_ACTION_UX_GUIDE.md` | Some immediate navigation paths may show feedback briefly before route transition. |
